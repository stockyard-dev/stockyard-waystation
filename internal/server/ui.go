...
async function load(){var r=await fetch(A+'/trips').then(function(r){return r.json()});items=r.trips||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.destination||'').toLowerCase().includes(q)||(i.start_date||'').toLowerCase().includes(q)||(i.end_date||'').toLowerCase().includes(q)});
...
if(i.destination)h+='<div class="item-sub">'+esc(i.destination)+'</div>';
...
if(i.start_date)h+='<span>'+esc(i.start_date)+'</span>';
if(i.end_date)h+='<span>'+esc(i.end_date)+'</span>';
if(i.budget)h+='<span>'+esc(i.budget)+'</span>';
if(i.itinerary)h+='<span>'+esc(i.itinerary)+'</span>';
if(i.status)h+='<span class="badge">'+esc(i.status)+'</span>';
if(i.notes)h+='<span>'+esc(i.notes)+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';

h+='</div>';});document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/trips/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",destination:"",start_date:"",end_date:"",budget:"",itinerary:"",status:"",notes:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' WAYSTATION</h2>';
h+='<div class="fr"><label>Name *</label><input id="f-name" value="'+esc(i.name)+'"></div>';
h+='<div class="fr"><label>Destination</label><input id="f-destination" value="'+esc(i.destination)+'"></div>';
h+='<div class="fr"><label>Start Date</label><input id="f-start-date" type="date" value="'+esc(i.start_date)+'"></div>';
h+='<div class="fr"><label>End Date</label><input id="f-end-date" type="date" value="'+esc(i.end_date)+'"></div>';
h+='<div class="fr"><label>Budget</label><input id="f-budget" value="'+esc(i.budget)+'"></div>';
h+='<div class="fr"><label>Itinerary</label><input id="f-itinerary" value="'+esc(i.itinerary)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),destination:document.getElementById('f-destination').value.trim(),start_date:document.getElementById('f-start-date').value.trim(),end_date:document.getElementById('f-end-date').value.trim(),budget:document.getElementById('f-budget').value.trim(),itinerary:document.getElementById('f-itinerary').value.trim(),status:document.getElementById('f-status').value.trim(),notes:document.getElementById('f-notes').value.trim()};
if(editId){await fetch(A+'/trips/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/trips',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
